 ...
 *
 * @since 4.2.7
 * @deprecated 6.0			no longer maintained - replaced by class avia_sc_custom_layout
 * @modified_by Günter
 */
if( ! defined( 'ABSPATH' ) ) {  exit;  }    // Exit if accessed directly


if( ! class_exists( 'avia_sc_postcontent', false ) )
{
	/**
	 * @deprecated 6.0			no longer maintained - replaced by class avia_sc_custom_layout
	 */
...
//			$this->config['modal_data']				= array( 'modal_class' => 'flexscreen' );
		...
		{
			$depr  = '<br />';
			$depr .= __( 'This element is kept for backwards compatibility and is no longer maintained. Consider to switch to new element &quot;Custom Layout&quot; which offers the same features.', 'avia_framework' );
			$depr .= '<br /><br />';
			...
			$this->elements = array(
				...
				array(
					...
					array(
							'name' 	=> __( 'ATTENTION - DEPRECATION NOTICE:', 'avia_framework' ),
							'desc' 	=> $depr,
							'type' 	=> 'heading',
							'description_class' => 'av-builder-note'
						),
...
//			$content = Av_Responsive_Images()->remove_loading_lazy_attributes( $content );
		...
		public function editor_element( $params )
		{
			...
			if( ! $entry instanceof WP_Post )
			{
				$params['innerHtml'].= "<div class='avia-element-description'>" . __( 'Allows you to display the content of a different entry (element is deprecated - use Custom Layout instead)', 'avia_framework' ) . '</div>';
			}
