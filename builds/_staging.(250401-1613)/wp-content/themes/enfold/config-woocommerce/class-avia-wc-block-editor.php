...
if( ! class_exists( 'Avia_WC_Block_Editor', false ) )
{
	class Avia_WC_Block_Editor
	{
		...
		public function handler_avf_create_styles_rules( $rules, $style_object )
		{
			...
			$colors = is_array( $avia_config['backend_colors']['color_set_var']['main_color'] ) ? $avia_config['backend_colors']['color_set_var']['main_color'] : $avia_config['backend_colors']['color_set']['main_color'];
			...
			$styles = array(
							...
							"color: {$colors['bg']}",
							"border-color: {$colors['button_border2']}"
						);
		...
		public function handler_avia_dynamic_css_output( $output, $color_set )
		{
			...
			{
				if( ! in_array( $section_key, array( 'main_color' ) ) )
				{
					continue;
				}
