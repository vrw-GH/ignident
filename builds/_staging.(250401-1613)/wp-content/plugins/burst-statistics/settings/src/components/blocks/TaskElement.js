...
const TaskElement = ( {notice, highLightField, onCloseTaskHandler} ) => {
    const handleClick = () => {
        highLightField( notice.highlight_field_id );
    };
    return (
        <div className="burst-task-element">
            <span className={'burst-task-status burst-' + notice.icon}>{ notice.label }</span>
            {'skeleton' !== notice.icon && <p className="burst-task-message" dangerouslySetInnerHTML={{__html: notice.msg}}></p>}
            {'skeleton' === notice.icon && <div className="burst-task-message" ><Icon name="loading" /></div>}
            {notice.url && (
                <a
                    target="_blank"
                    href={notice.url}
                    rel="noreferrer"
                >
                    {'sale' === notice.icon ? __( 'Get 40% Off', 'burst-statistics' ) : __( 'More info', 'burst-statistics' )}
                </a>
            )}

            {notice.highlight_field_id && <span className="burst-task-enable" onClick={handleClick}>{__( 'Fix', 'burst-statistics' )}</span> }
            {notice.plusone && <span className='burst-plusone'>1</span>}
            {notice.dismissible && 'completed' !== notice.status &&
                <div className="burst-task-dismiss">
                  <button type='button' data-id={notice.id} onClick={onCloseTaskHandler}>
