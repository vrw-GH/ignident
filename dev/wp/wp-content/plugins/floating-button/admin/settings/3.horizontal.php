...
$data = include( 'options/3.horizontal.php' );

$field = new CreateFields( $options, $data['opt'] );

$count = ( ! empty( $options['menu_2']['item_type'] ) ) ? count( $options['menu_2']['item_type'] ) : '0';
?>
	<div class="wpie-items__list" id="wpie-items-list">
		<?php if ( $count > 0 ) :
			for ( $i = 0; $i < $count; $i ++ ):
				...
				$item_parent = $options['menu_2']['item_sub'][ $i ] ?? 0;
				$item_class = '';
				...
				<details
					...
					<input type="hidden" name="param[menu_2][item_sub][]" class="wpie-item__parent"
		...
		<div class="wpie-fields">
			<button class="button button-primary wpie-add-button" type="button" data-template="template-button-2">
