...
$href = 'https://fonts.google.com/share?selection.family=Advent+Pro|Alice|Allerta|Arvo|Arimo|Antic|Audiowide|Bad+Script|Baumans|Bangers|Barlow|Bitter|Cabin|Cardo|Carme|Caveat|Coda|Codystar|Cormorant+Garamond|Comfortaa|Coustard|Gruppo|Damion|Dancing+Script|Dosis|EB+Garamond|Exo|Finger+Paint|Fira+Sans|Fjord+One|Flamenco|Great+Vibes|Heebo|Herr+Von+Muellerhoff|IBM+Plex+Serif|Inconsolata|Josefin+Sans|Josefin+Slab|Kameron|Karla|Kreon|Lato|Lobster|Lora|League+Script|Mate+SC|Marck+Script|Mako|Megrim|Merienda|Merriweather|Metrophobic|Molengo|Montserrat|Mulish|Nixie+One|Nobile|News+Cycle|Open+Sans|Orbitron|Oregano|Oswald|Pacifico|Parisienne|Petit+Formal+Script|Pinyon+Script|Playfair+Display|Podkova|Poiret+One|Poly|Press+Start+2P|PT+Sans|Quattrocento|Questrial|Quicksand|Raleway|Righteous|Roboto|Sacramento|Salsa|Signika+Negative|Source+Sans+3|Source+Serif+4|Sunshiney|Special+Elite|Tangerine|Tenor+Sans|Varela+Round|Work+Sans|Yellowtail&sort=alpha';

$web_safe = '<a href="https://www.cssfontstack.com/" target="_blank" rel="noopener noreferrer">' . __( 'devices', 'avia_framework' ) . '</a>';
$google_webfont  = '<a href="' . $href . '" target="_blank" rel="noopener noreferrer">' . __( 'Google Webkit Fonts', 'avia_framework' ) . '</a>';

$desc = sprintf( __( 'The heading font allows you to use a wide range of fonts for your headings. Upload your own fonts, use web save fonts (faster rendering, but not mandatory installed on all %1$s) or %2$s (more unique).', 'avia_framework' ), $web_safe, $google_webfont );
...
$desc = sprintf( __( 'Choose a font for your body text. Upload your own fonts, use web safe fonts (faster rendering, but not mandatory installed on all %1$s) or %2$s (more unique).', 'avia_framework' ),$web_safe, $google_webfont );
