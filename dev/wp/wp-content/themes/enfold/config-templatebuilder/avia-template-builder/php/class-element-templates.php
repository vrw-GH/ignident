...
if( ! class_exists( 'aviaElementTemplates', false ) )
{
	class aviaElementTemplates
	{
		...
		protected function activate_filters()
		{
			...
			add_filter( 'avf_cpt_support_post_types', [ $this, 'handler_avf_cpt_support_post_types' ], 10, 1 );
			...
			add_filter( 'avf_ignore_duplicate_post_types', array( $this, 'handler_avf_ignore_duplicate_post_types' ), 10, 1 );
		...
		/**
		 * Filter to force usage of ALB for alb_elements posts. This will also hide the switch button with CSS
		 *
		 * @since 6.0
		...
		/**
		 * Remove our CPT from list to allow modify edit table and default term settings
		 * We handle in this class.
		 *
		 * @since 6.0
		 * @param WP_Post_Type[] $pt_objs
		 * @return array
		 */
		public function handler_avf_cpt_support_post_types( $pt_objs )
		{
			if( ! empty( $pt_objs ) )
			{
				unset( $pt_objs[ $this->get_post_type() ] );
			}
