(function ($) {
	...
	$.fn.avia_sc_gallery = function (options) {
		return this.each(function () {
			...
			// Filmstrip mouse drag for desktop
			if (gallery.hasClass('avia-gallery-filmstrip')) {
				var strip = gallery.find('.avia-gallery-thumb'),
					isDown = false,
					startX = 0,
					scrollLeft = 0,
					didDrag = false;
				...
				strip.on('mousedown', function (e) {
					isDown = true;
					didDrag = false;
					strip.addClass('av-dragging');
					startX = e.pageX - strip.offset().left;
					scrollLeft = strip[0].scrollLeft;
				});

				strip.on('mouseleave mouseup', function () {
					isDown = false;
					strip.removeClass('av-dragging');
				});

				strip.on('mousemove', function (e) {
					if (!isDown) return;

					e.preventDefault();
					didDrag = true;

					var x = e.pageX - strip.offset().left;

					strip[0].scrollLeft = scrollLeft - (x - startX);
				});

				// Prevent lightbox from opening when user was dragging
				strip.on('click', 'a', function (e) {
					if (didDrag) {
						e.preventDefault();
						e.stopPropagation();
