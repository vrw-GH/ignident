...
if( ! class_exists( 'avia_sc_image_hotspots', false ) )
{
	class avia_sc_image_hotspots extends aviaShortcodeTemplate
	{
		...
		protected function register_dynamic_templates()
		{
			...
			$desc_img  = __( 'Select a custom field with an attachment ID to replace the selected image above using these image settings (a fallback image must be selected !!!). Leave empty to use image above as fallback.', 'avia_framework' );
			$desc_img .= '<br /><strong>';
			$desc_img .=	__( 'KEEP IN MIND: Hotspot locations are always relative to source image - make sure to use images that need the same locations.','avia_framework' );
			$desc_img .= '</strong>';
		...
		public function editor_element( $params )
		{
			...
			$orig_attachment = ! empty( $attr['attachment'] ) ? $attr['attachment'] : '';
			$orig_src_dynamic = ! empty( $attr['src_dynamic'] ) ? $attr['src_dynamic'] : '';

			Avia_Dynamic_Content()->read( $attr, $this, $this->config['shortcode'], $content );

			if( ! empty( $attr['src_dynamic'] ) )
			{
				$attr['attachment'] = $attr['src_dynamic'];
			}

			$template = $this->update_template_lockable( 'src', "<img src='{{src}}' alt=''/>", $locked );
			$template_dyn = $this->update_template_lockable( 'src_dynamic', "<span class='tmpl-hide-on-empty'>Custom field content: {{src_dynamic}}</span>", $locked );
			$dyn_overlay = '';
			...
			if( ! empty( $attr['attachment'] ) && ! empty( $attr['attachment_size'] ) )
			{
				...
				//	provide a fallback in case dynamic image does not exist (e.g. custom field not set or in custom layout when no source post provided)
				if( empty( $img ) && $orig_attachment != $attr['attachment'] || ! empty( $orig_src_dynamic ) )
				{
					if( empty( $img ) )
					{
						$img = wp_get_attachment_image( $orig_attachment, $attr['attachment_size'] );
					}

					$dyn_overlay = "<span class='tmpl-hide-on-empty'>Custom field content: {{$orig_src_dynamic}}</span>";
				}
			}
			...
			$params['innerHtml'] .=		"<div class='avia_image_dynamic_overlay' {$template_dyn}>{$dyn_overlay}</div>";
		...
		protected function get_element_styles( array $args )
		{
			...
			foreach( $hotspots as $key => &$item )
			{
				...
				$item['attr'] = array_merge( $item_def, $item['attr'] );

				Avia_Dynamic_Content()->read( $item['attr'], $this, $this->config['shortcode_nested'][0], $item['content'] );
				$item['attr']['link'] = Avia_Dynamic_Content()->check_link( $item['attr']['link_dynamic'], $item['attr']['link'] );
			}
			...
			{
				$atts['attachment'] = $atts['src_dynamic'];
			}
